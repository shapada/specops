...
get_header(); ?>
  ...
  <?php
    ...
    if (strlen($featuredContent['Datasheets']['description']) > 135) {
		...
		var_dump( $featuredContent['Datasheets']['description']  ); die();
	}
  ?>
            ...
            if ($second_query->have_posts()): while ($second_query->have_posts()): $second_query->the_post();
              ...
              foreach ($resource_types as $resource_type):
                ...
                if (array_key_exists($resource_type->name, $theContent)) {
					...
					echo '<pre>'; </pre>var_dump( $theContent[$resource_type->name][$thisCount]['excerpt'] ); die();
					if( ! empty( $theContent[$resource_type->name][$thisCount]['excerpt'] ) ) {
						$excerpt_substr = substr( $theContent[$resource_type->name][$thisCount]['excerpt'], 0, 185 );
						$excerpt 		= substr( $excerpt_substr, 0, strrpos( $excerpt_substr, '' ) );
					}
                    $theContent[$resource_type->name][$thisCount]['excerpt'] = $excerpt;
