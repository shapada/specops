...
function trinity_excerpt_filter( $excerpt, $post = null, $limit ) {
	$id = 0;
	...
	$excerpt = substr( $theContent[$resource_type->name][$thisCount]['excerpt'], 0, 185 );
						$excerpt 		= substr( $excerpt_substr, 0, strrpos( $excerpt_substr, '' ) );
}
add_filter( 'get_the_excerpt', 'trinity_excerpt_filter' );
